...
# Global variables to store patient counts
current_day_initial_count = None  # Initial patient count at 12 AM for the current day
last_midnight_count = None  # Final patient count at midnight for the previous day
...
def generate_hourly_graph():
    ...
    # Robust datetime parsing with flexible time formats
    def parse_datetime(date_col, time_col):
    
    # Combine date and time columns with a space separator
        combined = df[date_col].astype(str).str.strip() + ' ' + df[time_col].astype(str).str.strip()
    
    # Parse the combined string into a datetime object, handling multiple formats
        return pd.to_datetime(
            combined,
            format='%Y-%m-%d %H:%M:%S',  # Try parsing with seconds first
            errors='coerce'
        ).fillna(
            pd.to_datetime(combined, format='%Y-%m-%d %H:%M', errors='coerce')  # Fallback to parsing without seconds
        )
    ...
    # Step 1: Get initial patient count at 12 AM
    if current_day_initial_count is None:
        # First run of the day: Calculate initial count from the entire dataset
        ...
        # Subsequent runs: Use the stored initial count for the current day
        prevc = current_day_initial_count
    ...
    # Step 4: Update the midnight count for the next day
    if current_hour == 23:  # At midnight (end of the day)
        last_midnight_count = prevc  # Store the final count for the next day
        current_day_initial_count = None  # Reset for the next day
...
def get_current_day_discharge_count(df):
    # Logic to calculate current day discharge count (you can modify it based on the requirements)
